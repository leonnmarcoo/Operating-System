...
public class DEVELALEONMARCO2B extends JFrame {
    ...
    private void calculateAndDisplayResults() {
        // Create a copy of processes for SJF scheduling
        ArrayList<ProcessData> remainingProcesses = new ArrayList<>(processes);
        ArrayList<ProcessData> executionOrder = new ArrayList<>();
        
        // Sort all processes by arrival time first
        ProcessData[] allProcesses = processes.toArray(new ProcessData[0]);
        Arrays.sort(allProcesses, Comparator.comparingInt(p -> p.arrivalTime));
        
        int currentTime = allProcesses[0].arrivalTime;
        ...
        while (!remainingProcesses.isEmpty()) {
            ...
            if (availableProcesses.isEmpty()) {
                // No process has arrived yet, move time to next earliest arrival
                int nextArrival = Integer.MAX_VALUE;
                for (ProcessData process : remainingProcesses) {
                    if (process.arrivalTime > currentTime && process.arrivalTime < nextArrival) {
            ...
            // Find the process with shortest burst time (SJF)
            // In case of tie, select the one that arrived first (FCFS tie-breaking)
            ProcessData shortestJob = availableProcesses.get(0);
        ...
        generateGanttChart(executionOrder.toArray(new ProcessData[0]));
    }
    
    private void generateGanttChart(ProcessData[] executionOrder) {
        StringBuilder ganttChart = new StringBuilder();
        StringBuilder timelineNumbers = new StringBuilder();
        
        ganttChart.append("Gantt Chart: |");
        for (ProcessData process : executionOrder) {
        ...
        // Start from the first process's arrival time
        int startTime = executionOrder[0].arrivalTime;
        timelineNumbers.append(String.format("%-5s", startTime));
        
        // Add completion times for each process in execution order
        for (ProcessData process : executionOrder) {
            timelineNumbers.append(String.format("%-5s", process.completionTime));
        }
    ...
    public static void main(String[] args) {
        SwingUtilities.invokeLater(() -> {
            new DEVELALEONMARCO2B().setVisible(true);
        });
    }
}